...
module.exports = VmSubprovider; // handles the following RPC methods:
//   eth_call
//   eth_estimateGas
...
function VmSubprovider(opts) {
  var self = this;
  self.opts = opts || {};
  self.methods = ['eth_call', 'eth_estimateGas']; // set initialization blocker
...
VmSubprovider.prototype.setEngine = function (engine) {
  var self = this;
  Subprovider.prototype.setEngine.call(self, engine); // unblock initialization after first block
...
VmSubprovider.prototype.handleRequest = function (payload, next, end) {
  var self = this; // skip unrelated methods
  ...
  doWhilst(function (callback) {
    ...
    self.runVm(payload, function (err, results) {
      ...
      if (err || gasUsed === 0) {
        ...
        hi = mid; // Perf improvement: stop the binary search when the difference in gas between two iterations
...
VmSubprovider.prototype.runVm = function (payload, cb) {
  ...
  var blockNumber = ethUtil.addHexPrefix(blockData.number.toString('hex')); // create vm with state lookup intercepted
  ...
  var txParams = payload.params[0]; // console.log('params:', payload.params)
  ...
  var fakeTx = Object.create(tx); // override getSenderAddress
...
function blockFromBlockData(blockData) {
  var block = new Block(); // block.header.hash = ethUtil.addHexPrefix(blockData.hash.toString('hex'))
